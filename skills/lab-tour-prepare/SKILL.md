...
5. **兴趣匹配与路线排序**：
   - 为每个导览区域（入口、工位区、人形机器人展区、仿真与生活模拟区等）计算与访客兴趣的匹配度得分（0-10分）
   - 根据匹配度动态调整参观顺序，高匹配度区域优先安排，低匹配度区域可简化或后置
   - 生成最优导航路径，减少重复移动，提升导览流畅度
6. **生成定制化大纲**：基于访客兴趣分析结果和动态路线顺序，在[**doc-format**](./references/doc-format.md)的基础上调整章节顺序和内容权重，对与访客匹配度高的内容重点展开，相关性低的内容简化，每个章节开头增加对应的导航动作指令（如【导航到工位区】）
7. 结合母版内容、公共项目信息、访客个性化大纲，用 LLM 生成结构化 Markdown 导览稿，每个章节明确标记对应的导航点位
8. **openclaw-lark** `feishu_create_doc`(或等价) 新建 Doc,标题建议:`{日期}-{访客简称}-导览`。
9. 回复用户:**新 Doc 链接**、建议的**现场唤醒句**(写入 doc 末尾「唤醒」一节),并提示执行现场导览时调用 skill **`lab-tour-run`** 并传入该 `document_id`/链接。
=======
